...
//!
//! Fetches upcoming/running/recent matches for the two esports titles served by
//! PandaScore — CS2 (under the `/csgo/` path prefix) and LoL (under `/lol/`) —
...
mod brackets;
pub use brackets::{fetch_bracket, fetch_standings, fetch_tournament_meta};
...
/// The PandaScore URL path segment for an esports title. Only CS2 and LoL are
/// served by PandaScore; every other `Sport` is fetched from its own source and
/// never reaches this client.
fn game_path(sport: Sport) -> &'static str {
    match sport {
        Sport::Cs2 => "csgo",
        Sport::Lol => "lol",
        other => unreachable!("game_path is PandaScore-only (CS2/LoL); got {other:?}"),
    }
}
